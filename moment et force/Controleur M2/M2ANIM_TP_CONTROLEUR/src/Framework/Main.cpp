...
static void sKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_SPACE: // Launch a ball
			...
			break;
		case GLFW_KEY_0:
			if(app){app->setposition(0);}
			break;
		case GLFW_KEY_1:
			if(app){app->setposition(1);}
			break;
		case GLFW_KEY_2:
			if(app){app->setposition(2);}
			break;
		case GLFW_KEY_3:
			if(app){app->setposition(3);}
			break;
		case GLFW_KEY_4:
			if(app){app->setposition(4);}
			break;
        /*
...
int main(int, char**) {
	...
	while (!glfwWindowShouldClose(mainWindow)) {
		...
		if (settings.optimization) {
                ...
                // Si on a d�pass� le temps imparti pour une simulation
                if (currentTime - startTime >= settings.optiDuration) {
                        // Incr�ment du nombre d'it�ration
                        m_optiData.nbIter++;
                        // Arr�t de l'optimisation si limite d'it�rations atteinte
                        ...
                        // Erreur cumul�e de la simulation (ex. moments ou vitesses articulaires)
                        float errorCumulated = m_optiData.currentCost / 3000.0;
                        // Somme des erreurs pond�r�es
                        float totalError = errorDistance + errorCumulated;
                        // Si le bip�de n'est pas tomb� et que le co�t est meilleur
                        if (!app->BipedHasFallen() && totalError < m_optiData.bestCost) {
                            ...
                            // Recopie des param�tres courants dans les meilleurs param�tres
                        ...
                        // Modification des meilleurs param�tres
                        for (int i=0; i<m_optiData.nbParam; i++) {
                                // Signe al�atoire (ie. incr�ment ou d�cr�ment)
                                int signe = (rand() % 2 == 0) ? -1 : 1;
                                // Valeur max de l'incr�ment (diminue avec les it�rations)
                        ...
                        // Application des modifications sur les param�tres de l'optimisation
                        app->setOptimizationData(m_optiData.currentParam);
                        // Remize � z�ro du temps
                        startTime = currentTime;
                }
                else {
                    // Si on a pas d�pass� le temps imparti, on met � jour le cout cumul�

...
void Application::setOptimizationData(const float * data) {
    /*
    Dans cette proc�dure on affecte aux param�tres � optimiser le contenu du tableau data
    Ex. m_biped->getStateMachine()->m_states[0].targetAngles[0] = data[0];
    pour affecter la premi�re valeur du tableau au premier angle du premier �tat de la machine � �tats
    ou
   ...
   int size=m_biped->getStateMachine()->m_states.size();
   for(int j=0;j<size;j++)
   for(unsigned int i=0;i<m_biped->getStateMachine()->m_states[0].targetAngles.size();i++)
   {
		m_biped->getStateMachine()->m_states[j].targetAngles[i]=data[j*size+i];
   }
}

void Application::getOptimizationData(float * data) {
    /*
    Dans cette proc�dure on affecte dans le tableau les valeurs des param�tres � optimiser
    Ex. data[0] = m_biped->getStateMachine()->m_states[0].targetAngles[0];
    pour affecter la valeur du premier angle du premier �tat de la machine � �tats � la premi�re valeur du tableau
    ou
   ...
   for(unsigned int i=0;i<m_biped->getStateMachine()->m_states[0].targetAngles.size();i++)
   {
		data[j*size+i]=m_biped->getStateMachine()->m_states[j].targetAngles[i];
   }
}
...
void Application::setposition(int i)
{
	m_biped->setMachine(i);
}

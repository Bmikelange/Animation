...
mb_cullface(false), // Par defaut - gestion des faces cachees
...
b_draw_axe(true) // Par defaut - affiche les axes
{
    ...
    Terrain=read_mesh("data/modèles/terrain2.obj");
    for(int i=0;i<Terrain.normals().size();i++)
    {
        Terrain.normal(i,-(Vector)Terrain.normals()[i]);
    }
    arbre=read_mesh("data/modèles/arbre.obj");
    for(int i=0;i<arbre.normals().size();i++)
    {
        arbre.normal(i,-(Vector)arbre.normals()[i]);
    }
    table=read_mesh("data/modèles/cube.obj");
    for(int i=0;i<table.normals().size();i++)
    {
        table.normal(i,-(Vector)table.normals()[i]);
    }
    sphere=read_mesh("data/modèles/sphere.obj");
    for(int i=0;i<sphere.normals().size();i++)
    {
        sphere.normal(i,-(Vector)sphere.normals()[i]);
    }
    TextureTerrain= read_texture(0, "data/modèles/textureTerrain.png");
    TextureArbre= read_texture(0, "data/modèles/textureArbre.png");
...
int Viewer::render( )
{
    ...
    for(e=_Simu->_enfants.begin(); e!=_Simu->_enfants.end(); e++)
    {
        ...
        draw(Terrain, Translation(0,-0.3,0)*Scale(5,2,5), m_camera,TextureTerrain);
        draw(arbre, Translation(5,0,1)*Scale(0.5,0.5,0.5), m_camera,TextureArbre);
        draw(arbre, Translation(-3,0,1)*Scale(0.5,0.5,0.5), m_camera,TextureArbre);
        draw(arbre, Translation(1,0,5)*Scale(0.5,0.5,0.5), m_camera,TextureArbre);
        draw(arbre, Translation(1,0,-3)*Scale(0.5,0.5,0.5), m_camera,TextureArbre);
        draw(table, Translation(0.5,1.5,0.5)*Scale(0.5,0.5,0.5), m_camera);
        draw(sphere, Translation(-0.5,3.5,-0.5)*Scale(0.5,0.5,0.5), m_camera);

...
void ObjetSimuleRigidBody::initMeshObjet()
{
    ...
    /* Parcours des sommets pour creer les sommets dans le Mesh */
    ...
    /* Parcours des facettes pour creer les triangles */
    for(unsigned int i=0; i<_VISize; i=i+3)
    {
...
void ObjetSimuleRigidBody::updateVertex()
{
    ...
    for(unsigned int i=0; i<_Nb_Sommets; i++)
    {
        m_ObjetSimule.vertex(i,vec3(P[i].x,P[i].y,P[i].z));
    }
}
...
void ObjetSimuleRigidBody::Simulation(Vector gravite, float viscosite, int Tps)
{
    ...
    Collision();
    CollisionSphere(Point(0,0,0),0.5,Translation(-0.5,3.5,-0.5)*Scale(0.5,0.5,0.5));
    CollisionCube(Point(-1,-1,-1),Point(1,1,1),Translation(0.5,1.5,0.5)*Scale(0.5,0.5,0.5));
